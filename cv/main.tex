...
\begin{minipage}[t]{0.5\textwidth} % 50% of the page for the skills bar chart
  ...
  \begin{barchart}{5.5}
    \baritem{Ruby}{100}
    \baritem{TypeScript/JavaScript}{92}
    \baritem{Rust}{90}
    \baritem{HTML/CSS}{87}
    \baritem{PostgreSQL}{78}
    \baritem{Shell}{70}
...
\begin{entrylist}
  ...
  \entry
    {Feb 2023 -- Jul 2026\\\footnotesize{via Devbox}}
    {Software developer}
    {Instabee}
    ...
    because data needs to be ingested from many sources in order to generate
    ...
    \texttt{Node.JS}\slashsep\texttt{TypeScript}\slashsep\texttt{PostgreSQL}\slashsep\texttt{MongoDB}\slashsep\texttt{MySQL}\slashsep\texttt{Google
    Cloud}\slashsep\texttt{Terraform}\slashsep\texttt{Next.js\slashsep\texttt{Netsuite}}\slashsep\texttt{Tanstack}\slashsep\texttt{Data Ingestion}\\
  ...
  \entry
    ...
    \textit{Health Integrator}.\\
      ...
      Native}\slashsep\texttt{Ruby on
      Rails}\slashsep\texttt{PostgreSQL}\slashsep\texttt{Google
  ...
  \entry
    ...
    Since the beginning of 2023 I was working 50\% of my time here, split with
    \textit{Savant}.\\
      \texttt{Node.JS}\slashsep\texttt{TypeScript}\slashsep\texttt{Ruby}\slashsep\texttt{Remix
      aka React Router}\slashsep\texttt{Ruby on
      Rails}\slashsep\texttt{PostgreSQL}\slashsep\texttt{Kubernetes}\slashsep\texttt{Google
  ...
  \entry
    ...
    {Apoex deals with medicine and custom medication manufacturing. While
      ...
      medication.\\
      \texttt{Ruby}\slashsep\texttt{PostgreSQL}\slashsep\texttt{MSSQL}\slashsep\texttt{Vue}}
  \entry
    {Mar 2014 -- Jun 2020}
    {Software developer}
    {Hemnet}
    {Hemnet is Sweden's largest property portal and one of the largest websites
      ...
      endpoints, among other things. I have been designing and implementing the
      API used by broker systems to send listing data to us. I was also a key
      part of the team that implemented a fulltext search engine using
      Elasticsearch without any downtime or interruption in service in order to
      replace old SQL-based searching. We also planned the replacement of
      SQL-based watchlists with Elasticsearch Percolate queries.\\ I've also
      been
      working in other teams, shipping user-facing features and worked a lot on
      developer training.\\
      \texttt{Ruby}\slashsep\texttt{Ruby on Rails}\slashsep\texttt{Elasticsearch}\slashsep\texttt{Lucene}\slashsep\texttt{PostgreSQL}\slashsep\texttt{GraphQL}\slashsep\texttt{React}\slashsep\texttt{Redis}}
  \entry
  ...
  \entry
    ...
    {This company was working on search products that indexed most Swedish
      ...
      after you've found what you were looking for.\\ We built a custom backend
      for search based on \texttt{Apache Solr} and \texttt{Lucene}, managing
      merging and rebalancing of indices and shards. Later after
      \texttt{Elasticsearch} was released, we moved over part of the backend to
      using that instead.\\ Newsline Group was later and merged with Bisnode.\\
...
\begin{minipage}[t]{0.3\textwidth} %
  ...
  \begin{itemize}
      ...
      \item Python {\footnotesize (entry-level)}
      ...
      \item Terraform {\footnotesize (intermediate)}
  \end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.3\textwidth}
...
\begin{minipage}[t]{0.3\textwidth}
  ...
  \begin{itemize}
      ...
      \item Kubernetes
      \item Elasticsearch / Open Search
      \item PostgreSQL
      \item Redis / Valkey
  \end{itemize}
\end{minipage}
